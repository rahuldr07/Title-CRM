...
describe('the ledger identity', () => {
  ...
  it('starts from nothing and ends at the balance', () => {
    ...
    expect(pettyBalance(PETTY)).toBe(led.at(-1)?.after)
  })
...
describe('what a count is measured against', () => {
  ...
  it('reports the ledger as it stood, not as it ended', () => {
    const led = pettyLedger(PETTY)
    const third = must(led[2], 'a third ledger line')
...
describe('the ledger export', () => {
  it('writes a header, then every row with its balance before and after', () => {
    const rows = ledgerCsvRows(
      pettyLedger([
        entry({ id: 'A', d: new Date(2026, 6, 1), kind: 'credit', what: 'Float', amt: 5000, ref: 'V-1' }),
        entry({ id: 'B', d: new Date(2026, 6, 2), kind: 'debit', what: 'Tea', amt: 120, by: 'Ravi', ref: 'V-2', receipt: false }),
      ]),
    )
    expect(rows).toEqual([
      ['Date', 'What for', 'Who', 'Credit', 'Debit', 'Balance before', 'Balance after', 'Voucher', 'Receipt held'],
      [fmtDate(new Date(2026, 6, 1)), 'Float', 'Somebody', 5000, '', 0, 5000, 'V-1', 'yes'],
      [fmtDate(new Date(2026, 6, 2)), 'Tea', 'Ravi', '', 120, 5000, 4880, 'V-2', 'no'],
    ])
  })

  it('is only the header for an empty box', () => {
    expect(ledgerCsvRows([])).toHaveLength(1)
  })
})

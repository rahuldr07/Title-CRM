...
const person = (over: Partial<Person>): Person =>
  ({ ...must(STAFF[0], 'a seeded person'), id: 'x', n: 'Test Person', active: true, ...over })
...
describe('what it says', () => {
  const at = (doj: string) => must(celebrationsOn([person({ doj, dob: '' })], on(2026, 8, 3))[0], `a celebration for ${doj}`)

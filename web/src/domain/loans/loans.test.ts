...
describe('loanDeductionsFor', () => {
  ...
  it(
    'shows zero, not a live re-derivation, for someone with nothing recorded in a month whose run ' +
      'has closed',
    () => {
      ...
      expect(loanDeductionsFor('zz', 'Aug 2026', [active], [othersPayment], true)).toEqual([])
    },
  )

  it('in a month still open, keeps a recorded instalment and reads an unrecorded loan live', () => {
    const paidOne = loan({ id: 'A', who: 'zz', st: 'active', amt: 20000, emi: 4000, paid: 4000 })
    const newOne = loan({ id: 'B', who: 'zz', st: 'active', amt: 9000, emi: 3000, paid: 0 })
    const recorded: LoanPayment = { id: 'PM1', loanId: 'A', mn: 'Aug 2026', amt: 4000, at: new Date(2026, 7, 28) }
    const deds = loanDeductionsFor('zz', 'Aug 2026', [paidOne, newOne], [recorded])
    expect(deds.map((d) => [d.loan.id, d.amount])).toEqual([
      ['A', 4000],
      ['B', 3000],
    ])
  })

  it('first recovers a loan the month after it was taken, where its schedule starts', () => {
    const l = loan({ who: 'zz', st: 'active', amt: 10000, emi: 5000, takenOn: new Date(2026, 6, 20) })
    expect(loanDeductionsFor('zz', 'Jul 2026', [l], [])).toEqual([])
    expect(loanDeductionsFor('zz', 'Aug 2026', [l], [])).toEqual([{ loan: l, amount: 5000 }])
    expect(scheduleFor(l, [])[0]?.due).toBe('Aug 2026')
  })
...
describe('scheduleFor', () => {
  ...
  it('has no schedule before a loan is taken', () => {
    expect(scheduleFor(loan({}), [])).toEqual([])
  })
})
...
describe('nextPayrollMonth', () => {
  it('is the calendar month after now()', async () => {
    const { nextPayrollMonth } = await import('./loans')

...
import { currentTimeRules } from './timeRules'
import { pad, toIst, TZ2 } from '@/shared/lib/format'
...
export const ist = (t: string): string => `${t} ${TZ2}`

export const shiftHours = (sh: Pick<Shift, 'from' | 'to'>): string => `${sh.from}–${sh.to} ${TZ2}`
...
export function restCheck(m: DayMark | null | undefined, nowTime: string): RestState | null {
  ...
  if (done < currentTimeRules().restAfterMins) return null
  const took = m.breakMins ?? 0
  return took >= currentTimeRules().restMins
    ? { ok: true, msg: `${took} minutes of break taken across ${hm(done)} worked.` }
    : {
        ok: false,
        msg: `${hm(done)} worked with ${took ? `${took} minutes` : 'no'} break. A rest of ${currentTimeRules().restMins} minutes is required after ${currentTimeRules().restAfterMins / 60} hours.`,
      }
}

export const lateBy = (inAt: string, shift: Shift) => {
  const late = mins(inAt) - mins(shift.from)
  return late > currentTimeRules().lateGraceMins ? late : 0
}

...
import type { Exception } from './engine'
...
export function ruleWhen(r: Pick<Rule, 'when' | 'cond' | 'stages'>): string {
  if (r.stages?.length) return `stage is ${r.stages.map((s) => stageName(s)).join(' or ')}`
  if (r.when) return r.when
  const c: RuleCondition = r.cond ?? {}
  const bits: string[] = []
  if (c.stage) bits.push(`stage is ${stageName(c.stage)}`)

...
export const qcAverage = (rows: readonly { avg: number }[]): number | null =>
  rows.length ? rows.reduce((a, x) => a + x.avg, 0) / rows.length : null

export const averageText = (avg: number | null): string => (avg === null ? '—' : avg.toFixed(2))
...
export function reasonCounts(rated: readonly QcEntry[]): [string, number][] {
  const reasons: Record<string, number> = {}
  for (const x of rated) if (x.crit && x.note) reasons[x.note] = (reasons[x.note] ?? 0) + 1

...
import { pad, usDate } from '@/shared/lib/format'
...
const rostered = () => currentStaff().filter((p) => p.dep.length && p.active !== false)

const WAS = [
  'No check-out recorded',
  'Checked in at 10:40 IST',
  'No punch at all',
  'Checked out at 13:10 IST',
] as const
const ASK = [
  'Worked 09:30 to 18:30 IST',
...
export function makeLateLog(): LateMark[] {
  ...
  for (let back = 29; back >= 0; back--) {
    ...
    if (HOLIDAYS.some((h) => h.d === usDate(d) && !h.opt)) continue

    list.forEach((p) => {
      if (r() > 0.11) return
      const sh = shiftByKey(p.shift)
      const late = 3 + Math.floor(r() * 74)
      if (late <= currentTimeRules().lateGraceMins) return
...
export function absencePattern(id: string): AbsencePattern {
  const taken = currentLeave().filter((l) => l.who === id && l.st === 'approved')

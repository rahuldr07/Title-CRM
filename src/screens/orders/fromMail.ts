import { CLIENTS, PRODUCTS, US_STATES } from '@/data/catalog'
...
/* Only an exact code or name: "Full Search" is not quietly taken to be "Full search plus". */
const productOf = (said: string) =>
  PRODUCTS.find((p) => p.id.toLowerCase() === said.toLowerCase() || p.n.toLowerCase() === said.toLowerCase())?.id

/**
 * What intake read from an emailed order, as the New order form's fields —
 * so "Review & create order" opens on the order rather than a blank form.
 * A field the message did not state is left out, for the form's own default.
 */
export function draftFromMail(m: MailItem): Partial<Draft> {
  ...
  if (sender && m.f.includes('·') && CLIENTS.some((c) => c.n === sender)) d.client = sender

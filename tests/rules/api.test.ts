...
import { ApiError, REQUEST_TIMEOUT_MS, fetchMemberships } from '@/lib/api'

/**
 * A request that never answers.
 *
 * The session waits on `/api/memberships` before it shows anything, so a
 * connection that hangs instead of refusing — a closed port on some machines,
 * an office firewall, a VPN — left the app on a blank screen for good. A
 * request has to end, one way or the other, so the session can fall back to
 * the seed build.
 */

/* A fetch that answers only when aborted, which is what a hung connection is. */

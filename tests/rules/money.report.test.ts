...
import { moneyByClient, moneyByMonth } from '@/screens/reports/money'
import { balance, invoicesNow } from '@/lib/invoices'
import { r2 } from '@/lib/format'

/**
 * How the business did in money, month by month and client by client.
 *
 * Reports had no revenue or collections anywhere, so the owner could not answer
 * "how did we do this month". Every figure here is the invoice register's, so
 * the report and Invoicing cannot disagree.
 */
...
describe('money by client', () => {
  it('ranks clients by what they were billed', () => {
    ...
    expect(c.every((x, i) => i === 0 || c[i - 1].billed >= x.billed)).toBe(true)
  })
})
